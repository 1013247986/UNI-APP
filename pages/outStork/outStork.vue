<template>
	<view class="main">
		<view class="boxlist">
			<view class="boxOne" v-for="(item, index) in list" :key="index" @click="TO(item)">
				<view class="boxOne-left">
					<view class="text">
						出库记录单编号：
						<view class="text-num">{{ item.warehouseOutCode }}</view>
					</view>
					<view class="text">
						仓库：
					...
					<view class="text">
						操作员：
						<view class="text-cont">{{ item.lastModifyName }}</view>
					</view>
				</view>
...
export default {
	...
	methods: {
		...
		TO(item) {
			uni.navigateTo({
				url: `./outStorkDet/outStorkDet?info=${JSON.stringify(item)}`
			});
		},
		show() {
			this.loadText = '加载中...';
			this.urlRequest({
				url: `/operation-agency-service/warehouseOut/listWarehouseHead?access_token=${this.app.access_token}`,
				method: 'post',
				data: {
					isPage: true,
					pageNo: this.pageNo,
